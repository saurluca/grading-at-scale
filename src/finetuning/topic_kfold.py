...
def split_by_topics(
    ...
    print(f"Total task_ids: train={len(train_task_ids)}, val={len(val_task_ids)}")
    ...
    val_indices = [
        i for i, task_id in enumerate(train_data["task_id"]) if task_id in val_task_ids
    ]
    ...
    if not val_indices and len(train_indices) > 10:
        ...
        print(
            f"Warning: No validation set created, using {len(val_indices)} samples from train"
        )
    ...
    if out_of_fold_samples > 0:
        ...
        remaining_samples = out_of_fold_samples - (
            samples_per_label * len(labels_in_test)
        )
        ...
        if len(oof_indices) < out_of_fold_samples:
            remaining_indices = [
                i for i in range(len(test_data)) if i not in oof_indices
            ]
        ...
        train_with_oof = concatenate_datasets(
            [train_data.select(train_indices), oof_samples_dataset]
        )

        print(f"  Added {oof_samples_added} samples from test topics to training")
        print(
            f"  Test set reduced from {len(test_data)} to {len(remaining_test_indices)} samples"
        )
    ...
    print(
        ...
        + (
            f" (including {oof_samples_added} from test topics)"
            if oof_samples_added > 0
            else ""
        )
    )
...
def main() -> None:
    ...
    if train_topics_count not in [2, 3]:
        print(
            f"Warning: train_topics must be 2 or 3, got {train_topics_count}. Defaulting to 3."
        )
    ...
    train_topic_combinations = list(
        itertools.combinations(unique_topics, train_topics_count)
    )
    print(
    ...
    with mlflow.start_run(run_name=f"topic_kfold_{model_name.split('/')[-1]}"):
        ...
        for fold_idx, train_topics in enumerate(train_topic_combinations, 1):
            ...
            run_name = (
                f"fold_{fold_idx}_{'_'.join(train_topics)}_vs_{'_'.join(test_topics)}"
            )
            with mlflow.start_run(run_name=run_name, nested=True):
                ...
                try:
                    ...
                    ds_name = str(
                        getattr(cfg.dataset, "dataset_name", Path(dataset_csv).stem)
                    )
                    import warnings
                ...
                output_dir = str(
                    PROJECT_ROOT / cfg.output.dir / f"topic_kfold_fold_{fold_idx}"
                )
        ...
        print("-" * 80)
        print(
            f"{'Fold':<6} {'Train Topics':<30} {'Test Topics':<30} {'Overall F1':<12}"
        )
        print("-" * 80)
        ...
        for topic in sorted(unique_topics):
            if len(topic_performance[topic]) > 0:
                ...
                print(f"{topic:<20} {avg_f1:<20.4f} {std_f1:<15.4f} {n_folds:<10}")
                ...
                [
                    {
                        ...
                        **{
                            f"topic_{t}_weighted_f1": r["topic_f1_scores"].get(t, 0.0)
