...
def print_statistics(all_stats: dict, all_dfs: dict):
    ...
    for split in splits:
        print(f"\n{split.upper()} SPLIT:")
        print("-" * 120)
        print(f"{'Topic':<20} {'Questions':<12} {'Answers':<12} {'Correct':<12} {'Partial':<12} {'Incorrect':<12}")
        print("-" * 120)
        ...
        for topic in all_topics:
            if topic in split_stats:
                s = split_stats[topic]
                print(f"{topic:<20} {s['questions']:<12} {s['answers']:<12} "
        ...
        if split_df is not None:
            total = {
                ...
                'correct_answers': sum(s['correct_answers'] for s in split_stats.values()),
                'partially_correct_answers': sum(s['partially_correct_answers'] for s in split_stats.values()),
                'incorrect_answers': sum(s['incorrect_answers'] for s in split_stats.values())
            }
        else:
            total = {'questions': 0, 'answers': 0, 'correct_answers': 0, 
                    'partially_correct_answers': 0, 'incorrect_answers': 0}
        
        print("-" * 120)
        print(f"{'TOTAL':<20} {total['questions']:<12} {total['answers']:<12} "
    ...
    topic_totals = defaultdict(lambda: {
        'questions': set(),
        'answers': 0,
        'correct_answers': 0,
        'partially_correct_answers': 0,
    ...
    for topic in all_topics:
        if topic in topic_totals:
            ...
            print(f"{topic:<20} {len(t['questions']):<12} {t['answers']:<12} "
    ...
    if all_dfs_list:
        ...
        grand_total = {
            'questions': combined_df['task_id'].nunique(),  # Unique task_ids across all splits
            'answers': sum(t['answers'] for t in topic_totals.values()),
            'correct_answers': sum(t['correct_answers'] for t in topic_totals.values()),
            'partially_correct_answers': sum(t['partially_correct_answers'] for t in topic_totals.values()),
            'incorrect_answers': sum(t['incorrect_answers'] for t in topic_totals.values())
        }
    else:
        grand_total = {
            'questions': 0,
    ...
    print("-" * 120)
    print(f"{'GRAND TOTAL':<20} {grand_total['questions']:<12} {grand_total['answers']:<12} "
...
def verify_no_overlap(all_dfs: dict):
    ...
    if 'train' in task_id_sets and 'test' in task_id_sets:
        ...
        if train_test_overlap:
            overlaps.append(f"Train-Test: {len(train_test_overlap)} overlapping task_ids")
    ...
    if overlaps:
        ...
        for overlap in overlaps:
            print(f"   {overlap}")
        print("   This indicates the splits were not properly separated by questions.\n")
    else:
