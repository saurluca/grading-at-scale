...
tokenized_train = small_train.map(
    preprocess_function, batched=True, remove_columns=small_train.column_names
)
tokenized_eval = small_eval.map(
    preprocess_function, batched=True, remove_columns=small_eval.column_names
)
...
trainable_params = sum(
    p.numel() for p in regression_model.parameters() if p.requires_grad
)
...
# hub_model_id = "saurluca/Llama-3.2-1B-saf-slurm"
